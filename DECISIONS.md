...
**DB 비관적 락**은 정합성은 완벽하지만, 1000개의 트랜잭션이 동일 행에서 줄 서면 락 대기 시간이 누적되고 Connection Pool이 고갈된다. 실제로 300 VU 테스트에서 MySQL 데드락(Error 1213)이 발생해 500 에러가 터졌다.

**DB 낙관적 락**은 고경쟁 환경에서 대부분이 충돌 후 재시도하며 재시도 폭풍이 발생한다. 재시도 순서가 보장되지 않아 공정성도 깨진다.

**Redis Lua Script**는 Redis 싱글 스레드 위에서 Lua Script가 원자적으로 실행된다. 아래 5단계를 하나의 스크립트로 묶어 Race Condition 없이 처리한다.
...
Redis에 먼저 도달한 요청이 먼저 처리되므로 공정성이 자연스럽게 보장된다. 990개의 요청이 Redis에서 즉시 차단되어 DB까지 오지 않으므로 DB 부하가 최소화된다.
...
**1단계 — Redis 조기 차단**: 재고 소진 후 요청은 DB 미접근으로 즉시 반환.

**2단계 — DB 처리 최소화**: Redis 선점 성공한 10건만 DB 트랜잭션 실행. 락 경합 구간에 10개 스레드만 진입하므로 Lock contention 없음.

**3단계 — PG 호출 격리**: DB 락은 Booking INSERT 이후 해제. PG 호출(1~3초) 동안 DB 락을 들고 있지 않으므로 다른 요청 블로킹 없음. (DB 락 구간에 이미 10개만 있으므로 트랜잭션 분리 불필요)

**4단계 — PG Circuit Breaker**: 실패율 60% or 느린 호출(3초 이상) 50% 초과 시 회로 오픈. Spring 스레드가 PG 응답을 기다리며 Thread Pool이 고갈되는 것을 차단.
...
### 상황
Redis가 다운되면 재고 선점을 위한 모든 요청이 DB로 쏟아진다. 1000TPS 환경에서 Redis 없이 DB가 직접 받으면 Connection Pool 고갈 및 서버 다운으로 이어진다.
...
- 슬라이딩 윈도우 10건 중 실패율 50% or 느린 호출(500ms 이상) 80% 초과 → OPEN
- OPEN 시 즉시 fallback으로 전환, Redis 회복 대기 (10초 후 HALF_OPEN)

**Bulkhead (promotionDbFallback)**
- `max-concurrent-calls: 10`, `max-wait-duration: 0ms`
- DB Fallback 동시 진입을 인스턴스당 최대 10개로 제한
- 초과 요청은 즉시 `BulkheadFullException` → `SERVICE_UNAVAILABLE` 반환
...
**30개 경합이 DB에 문제가 없는 이유**: DB Fallback에서 실행하는 `decreaseStockById`는 단순 UPDATE 한 줄이라 락 보유 시간이 수 ms 수준이다. 30개 스레드가 줄 서더라도 마지막 스레드 대기 시간은 30 × 수ms = 약 60~100ms 수준이며, stock=0 이후 나머지는 `WHERE stock > 0` 조건으로 즉시 반환된다. 970개는 Bulkhead에서 이미 차단됐으므로 DB에 도달하지 않는다.

**정합성 보장**: `decreaseStockById`의 `WHERE stock > 0` 조건이 원자적으로 실행되므로 30개가 경합해도 초과판매가 발생하지 않는다.

**DB Fallback에서 추가로 하는 것**: `findWithLockById`로 `SELECT FOR UPDATE`를 먼저 실행해 프로모션 시간 유효성을 검증한 뒤 재고를 차감한다. 이는 Redis에서 하던 시간 검증을 DB 경로에서도 보장하기 위함이다.
...
결제가 됐는지 안 됐는지 모르는 상태에서 롤백하면 고객 돈이 빠져나갔는데 예약은 없는 최악의 상황이 된다. PENDING을 유지하면 이후 PG 웹훅이나 배치 스캐너로 결제 결과를 재확인해 COMPLETED 또는 FAILED로 전환할 수 있다.

Spring `@Transactional(noRollbackFor = PgUncertainException.class)`로 결과 불분명 예외 시에도 트랜잭션을 커밋한다.
...
**Retry에서 orderId를 PG 멱등성 키로 사용**하므로, 재시도해도 PG 측에서 중복 청구가 발생하지 않는다.
...
```
스레드 A가 gap lock을 들고 B의 행 락을 기다리고, 스레드 B가 행 락을 들고 A의 gap lock을 기다리는 순환 대기가 발생했다.
...
10개의 Redis 선점 성공 스레드가 1번 `promotion_room_type` 행에서 직렬화된다. 락 순서가 항상 동일하므로 순환 대기 자체가 불가능하다.

데드락 catch 처리는 땜질이다. MySQL이 롤백을 완료한 시점에 `catch`해봤자 이미 트랜잭션은 죽어있고 Spring이 commit을 시도하면 `Application exception overridden by commit exception`이 터진다. 구조 자체를 바꾸는 방향을 택했다.
...
JPA 1차 캐시(Persistence Context) 문제였다. 같은 트랜잭션 내에서 이미 캐시된 엔티티가 있으면 `SELECT FOR UPDATE`를 보내도 캐시에서 stale 값을 반환한다. 여러 스레드가 모두 `stock = 10`으로 읽고 `stock = 9`를 flush하면 마지막 쓰기만 살아남는다. 트랜잭션 종료 시 dirty check가 캐시의 stale 값을 DB에 덮어쓰는 것도 동일한 문제였다.
...
- `SET stock = stock - 1`: DB 현재 값 기준으로 차감, 캐시 우회
...
전략 패턴으로 `PaymentProcessor` 인터페이스를 정의하고 각 수단을 독립 빈으로 등록한다. `PaymentProcessorRegistry`가 타입별로 프로세서를 조회한다. 새 결제 수단 추가 시 구현 클래스 하나만 추가하면 된다.

복합 결제 허용 조합 검증(`validatePaymentCombination`)은 별도 `PaymentValidator`에 격리해, 조합 규칙 변경이 결제 실행 로직에 영향을 주지 않도록 분리했다.
...
### 상황
PG Circuit Breaker는 PG 호출 실패율이 60%를 넘으면 회로를 연다. 선착순 이벤트에서 카드 한도 초과 등의 결제 거절이 몰릴 수 있는데, 이를 PG 인프라 장애와 동일하게 집계하면 정상 사용자까지 결제가 차단된다.
...
카드 거절(`PgCardRejectedException`)은 PG 서버가 정상 응답한 것이므로 CB 실패 카운트에서 제외한다. 타임아웃이나 5xx는 PG 서버 문제이므로 정상 집계한다. 이를 통해 사용자 측 문제와 인프라 문제를 분리해 CB의 정확도를 높였다.
...
### 상황
Redis, PG, DB가 혼재하고 각 외부 시스템마다 장애 대응 전략이 다르다. 레이어드 아키텍처라면 Service가 `RedisTemplate`, `JpaRepository`, `FeignClient`에 직접 의존해 인프라 변경과 테스트가 어려워진다.
...
이 시스템은 Redis Fallback, PG Circuit Breaker, DB 비관적 락이 공존하는 복잡한 인프라 구조다. Service가 `RedisTemplate`에 직접 의존하면 Redis를 교체하거나 Fallback 전략을 바꿀 때 비즈니스 로직까지 건드려야 한다. 헥사고날로 OutputPort 인터페이스를 두면 Application 로직은 그대로 두고 Adapter만 바꿀 수 있다. 도메인 단위 테스트는 Spring Context 없이 순수 JUnit으로 실행 가능해 테스트 피드백이 빠르다. 초기 보일러플레이트 비용을 감수하고 장애 대응 로직의 격리와 테스트 용이성을 확보했다.
...
### 상황
Circuit Breaker, Retry, Bulkhead를 모두 적용해야 한다. 자체 구현 vs. Resilience4j 선택이 필요했다.

### 선택지
- **A. 자체 구현**: AtomicInteger, ConcurrentHashMap으로 직접 구현
- **B. Resilience4j**: Spring Boot Starter로 선언적 적용

### 왜 그렇게 판단했는지

자체 구현은 슬라이딩 윈도우, HALF_OPEN 상태 전환, 스레드 세이프한 카운터 관리를 모두 직접 구현해야 해서 버그 위험이 높다. Resilience4j는 `@CircuitBreaker`, `@Retry`, `@Bulkhead` 어노테이션 하나로 선언적으로 적용 가능하고, `application.yml`에서 파라미터를 관리해 코드 수정 없이 임계값을 조정할 수 있다. 프로덕션 검증된 라이브러리를 채택해 인프라 레이어에 집중하지 않고 비즈니스 로직에 집중했다.
